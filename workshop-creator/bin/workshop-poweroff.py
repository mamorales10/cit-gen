...
if len(sys.argv) < 2:
    print "Usage: python workshop-poweroff.py <input filename>"
    exit()
...
for vm in vmset.findall('vm'):
    ...
    for i in range(1, numClones + 1):
        ...
        try:
            powerOffCmd = [pathToVirtualBox, "controlvm", newvmName, "poweroff"]
            print("\Powering off VM: " + newvmName)
            print("\nexecuting: ")
            print(powerOffCmd)
            result = subprocess.check_output(powerOffCmd)
            print(result)
        except Exception:
            print "Could not poweroff VM:", newvmName
